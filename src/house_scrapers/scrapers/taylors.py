...
SITE_URL = "https://www.taylorsestateagents.co.uk"
SEARCH_URL = f"{SITE_URL}/properties/lettings"
SEARCH_API = f"{SITE_URL}/search.ljson"
PLACE_ID = "51e7c62e73dadaf60fef5493"
STORAGE_BLOB = "taylors/properties.json"
...
def parse_embedded_json(html: str, marker: str) -> dict[str, object]:
    ...
    if start < 0:
        raise RuntimeError(f"Taylors JSON marker not found: {marker}")
    value, _ = json.JSONDecoder().raw_decode(html[start + len(marker):].lstrip())
    if not isinstance(value, dict):
        raise RuntimeError(f"Taylors JSON marker is not an object: {marker}")
    return value
...
def candidate_offer(property_data: dict[str, object], config: ScraperConfig) -> dict[str, object] | None:
    ...
    if property_data.get("status") != "To let":
        return None
...
def api_url(config: ScraperConfig, page: int) -> str:
    query = urlencode({
        "channel": "lettings",
        "place_id": PLACE_ID,
        "count": 50,
        "fragment": f"up-to-{config.max_rent}/most-recent-first/page-{page}",
    }, safe="/")
...
def discover_offers(session: requests.Session, config: ScraperConfig, mode: str, now: datetime | None = None) -> Discovery:
    if mode not in MODES:
        ...
        raise ValueError(f"TAYLORS_MODE must be one of: {allowed}")
    ...
    page = 1
    while True:
        response = session.get(api_url(config, page), timeout=30)
        response.raise_for_status()
        payload = response.json()
        properties = payload.get("properties")
        pagination = payload.get("pagination")
        if not isinstance(properties, list) or not isinstance(pagination, dict):
            raise RuntimeError("Taylors search response has an invalid contract")
    ...
    if mode == "scrape_all" and total and len(seen) != total:
        raise RuntimeError(f"Taylors traversal incomplete: saw {len(seen)} of {total} adverts")
...
def detail_offer(session: requests.Session, candidate: dict[str, object], config: ScraperConfig) -> dict[str, object] | None:
    ...
    if not description:
        raise RuntimeError("Missing Taylors description for {}".format(candidate["link"]))
    address = str(candidate["address"])
    if not location_allowed(address, config):
        raise RuntimeError("Taylors detail location is excluded: {}".format(candidate["link"]))
...
def run() -> None:
    ...
    with http_session() as session:
        ...
        for index, candidate in enumerate(discovery.offers, start=1):
            ...
            if isinstance(existing_metadata, dict) and existing_metadata.get("source") == "taylors":
                continue
